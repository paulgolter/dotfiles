...
def main():
  ...
  # Call formatter.
  command = [binary, '-style', 'file', '-cursor', str(cursor)]
  if lines != ['-lines', 'all']:
    command += lines
  if fallback_style:
    command.extend(['-fallback-style', fallback_style])
  if vim.current.buffer.name:
    command.extend(['-assume-filename', vim.current.buffer.name]) # No longer using os.path.basename!

  if style != 'file':
    prevdir = os.getcwd()
    os.chdir(os.path.dirname(os.path.realpath(style)))
